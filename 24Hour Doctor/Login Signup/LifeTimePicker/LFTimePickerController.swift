...
open class LFTimePickerController: UIViewController {
    ...
    var firstRowIndex = 0
    var lastSelectedLeft = ""
    var lastSelectedRight = ""
    var strDay:String! = ""
    ...
    open var titleText: String = "Select Time"
    ...
    @objc fileprivate func butSave() {
        ...
        if isCustomTime {
            delegate?.didPickTime(time, end: time2, day: strDay, statsu: strOpenStatus)
        } else if timeType == .hour12 {
            
            delegate?.didPickTime(time + " \(self.lblAMPM.text!)", end: time2 + " \(self.lblAMPM2.text!)", day: strDay, statsu: strOpenStatus)
        } else {
            delegate?.didPickTime(time, end: time2, day: strDay, statsu: strOpenStatus)
        }
